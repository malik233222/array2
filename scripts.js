...
function getCategories() {
    ...
    for (let item of categories) {
        ...
        let continuedWord = word.slice(1);
        let finalWord = firstLetter + continuedWord;
        data += `<li><button class="dropdown-item" onclick="productsFilter('${item}')">${finalWord}</button></li>`
    };
...
function getProducts(mixP) {
    let products_html = document.getElementById('products');
    console.log(mixP,"mix");
    let data = '';
    for (let i = 0; i < mixP.length; i++) {
        data +=
            `
        <div class="col mb-5">
        <div class="card h-100">
            <!-- Sale badge-->
            <div class="badge bg-dark text-white position-absolute" style="top: 0.5rem; right: 0.5rem">${mixP[i].category}
            </div>
            <!-- Product image-->
            <img class="card-img-top custom-img" src="${mixP[i].image}" alt="..." />
            <!-- Product details-->
            <div class="card-body p-4">
                <div class="text-center">
                    <!-- Product name-->
                    <h5 class="fw-bolder">${mixP[i].title}</h5>
                    <!-- Product reviews-->
                    <div class="d-flex justify-content-center small text-warning mb-2">
                      
                       ${generateRating(mixP[i].rating.rate)}
                    </div>
                    <p>  </p>
                    <!-- Product price-->
                    ${mixP[i].price}
